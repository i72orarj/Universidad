...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2) {
        printf("unnamed_sem_t_ok y/n (use semaphore)\n");
        return 1;
    }
    ...
    *counter = 0.0;
    ...
    key = ftok("unnamed_sem_t_ok.c", 2);
    if ((shmid = shmget(key, sizeof(sem_t), IPC_CREAT | 0777)) == -1)
        exit(1);

    mutex = (sem_t *) shmat(shmid, NULL, 0);    //este puntero apunta a una zona de memoria compartida, cuando los demas procesos obtengan una copia, podran trabajar sobre la zona a la que apunta
    sem_init(mutex, 1, 1); 
