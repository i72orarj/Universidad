	...
	for(i=0; i<2; i++) {
		...
		if(pid == -1) {
			...
			if(i==0) {
				...
				//como un array. Se pone & ya que no sabemos lso arguemtnos que le vamos a pasar.
			}
		}	
	}
...
int hijoCalculadora(char * comando) {
	...
	execlp(comando, comando,/*no se pone porque es solo un dato*/ NULL); //Sirve para ejecutar un comando en concreto. Ejecuta comandos.
	return 0;
}
...
int hijoGedit(char ** argv) {
	...
	execvp(*argv, argv); //Primero lleva *, segundo no.
