...
void * suma_aleatorios(void * parametro)
{
	...
	int aleatorio1;
	int aleatorio2;
	int *suma; //tiene que ser tipo puntero por que se trabaja con referencia
	...
	*suma=aleatorio1+aleatorio2;
	...
	pthread_exit( (void *) suma); //esta vez devolvemos la variable
}
...
int main(int argc, char **argv)
{
	...
	srand ((int) time(&t)); // podemos poner srand(time(NULL));
	...
	n_hebras=atoi(argv[1]); //pasar la cadena de linea de ordenes en un entero

	//Creamos las hebras.
	//un for para el create otro for para el join
	for(i=0;i<n_hebras;i++)
	{
		hebras_num[i]=i+1;
		error = pthread_create( &(hebras_id[i]), NULL, (void *) suma_aleatorios, (void *) &hebras_num[i]);
		/*
		cuando quiera pasarle numeros a la funcion, las metere un vector y le pasare cada uno de los numeros del vector
	...
	for(i=0;i<n_hebras;i++)
	{
		error = pthread_join(hebras_id[i], (void **) &puntero_a_suma); //siempre que se devuelve es (void **) y un & con el nombre
