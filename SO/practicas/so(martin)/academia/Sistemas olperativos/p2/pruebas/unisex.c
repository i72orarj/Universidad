...
#include <stdlib.h>
#include <pthread.h>
#include <semaphore.h> //LIBRERIA PARA SEMAFOROS
#include <unistd.h>
...
void checkNULL(int* f, char msg[200]); //Comprueba si hay fallos en las reservas de memoria
 ...
 //SEMAFOROS SE DECLARAN GLOBALES, Y EL TIPO DE DATO DE LOS SEMAFOTOS ES SEM_T

sem_t empty; //Semáforo para indicar que el servicio está vacío. Inicializado a 1
sem_t male; //Sección crítica para los hilos de hombres. Inicializado a 1 - SOLO PUEDE ENTRAR UN HOMBRE(EN ESTE CASO, Y UNA MUJER EN EL OTRO) POR LA PUERTA A LA VEZ	
sem_t female; //Sección crítica para los hilos de mujeres. Inicializado a 1
sem_t male_count; //Cantidad de hombres que podrían entrar al servicio. Inicializado a CAPACIDAD - SI UN HOMBRE ESTA INCREMENTANDO CUENTA, NO PUEDE HABER OTRO DECREMENTANDO Y CON LAS MUJERES IGUAL.
sem_t female_count; //Cantidad de mujeres que podrían entrar servicio. Inicializado a CAPACIDAD
sem_t turnstile; //Indica si es el turno de hombres o mujeres para entrar en el baño. Inicializado a 1 - SI NO HAY MUJERES, PUEDEN ENTRAR HOMBRES Y VICEVERSA
...
int main()
{
  pthread_t *thr_cola = NULL; //vector dinamico vacio
  int *vectorID = NULL; // vector dinamico para decirte yo soy hilo 0, yo soy hilo 1..
  int i, cola = 2*HILOS;
 
  srand(time(0));
  
  //Reservando memoria para vectores
  vectorID = (int*)calloc(cola, sizeof(int));
 thr_cola = (pthread_t*)calloc(cola, sizeof(pthread_t));
  
  //Inicializando semáforos.
  checkError(sem_init(&empty, 0, 1), "Error al inicializar empty. Saliendo."); // para crear hilo sem_init(nombre de semaforo, 0, valor al que inicializas el semaforo)
  checkError(sem_init(&male, 0, 1), "Error al inicializar male. Saliendo.");
  checkError(sem_init(&female, 0, 1), "Error al inicializar female. Saliendo.");
  checkError(sem_init(&male_count, 0, CAPACIDAD), "Error al inicializar male_count. Saliendo."); // al ser un contador pones el nombre del semaforo, 0 y la cantidad hasta la que puede llegar, en este caso CAPACIDAD QUE ESTA CREADO COMO VARIABLE GLOBAL DE VALOR 3
