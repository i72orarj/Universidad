...
//Versión b: Cuando la última termina de leer del buffer, ésta despierta al escritor para que lo rellene con nuevos datos.
...
void * f_copiador(void * parametro){
	...
	do{
		...
		if (procesosterminados==n_hebras){

			pthread_cond_broadcast(&seguirleyendo);	//Despierta al escritor para informarle de que todos los procesos han terminado.
