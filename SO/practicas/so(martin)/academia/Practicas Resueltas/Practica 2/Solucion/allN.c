/*
   Solución al problema del productor/consumidor en el cual todos los
   consumidores consument los ítems.
 */
...
main (void)
{
  ...
  for (i = 0; i < C; i++)
    {
      v[i] = i;
      pthread_create (&consumidor[i], &atributos, Consumidor, &v[i]);
    }
