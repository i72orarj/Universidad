...
int main (int argc, char **argv)
{
   ...
   opterr = 0;
    ...
    while ((c = getopt (argc, argv, "abc:")) != -1)
    {
        ...
        //   - optopt: recoge el valor de la opcion cuando es desconocida (?) o
                      //INCOMPLETA respecto a las opciones indicadas.
        switch (c)
        {
		     case 'a':
		     ...
		     case '?': //Opcion no reconocida o INCOMPLETA (sin argumento).
                  //Probar tambien la diferencia entre ejecutar %$>./a.out m   ó   %$>./a.out -m
