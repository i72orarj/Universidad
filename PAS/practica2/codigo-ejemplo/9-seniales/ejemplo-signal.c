...
{
    printf("Capturé la señal SIGINT %d y no salgo!\n",signal);
    return;
}
...
{
    printf("Capturé la señal SIGHUP %d y no salgo!\n",signal);
    return;
}
...
{
    int i;
    printf("Capturé la señal SIGTERM %d y voy a salir de manera ordenada\n",signal);
    for(i=0; i<3; i++)
    {
...
{
    printf("Capturé la señal SIGKILL %d y no salgo!\n",signal);
    return;
}
