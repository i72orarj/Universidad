...
int main(int argc, char const *argv[]) {
  ...
  if(argc!=5){
    ...
    int* vectorAleatoriosPares=rellenaVector(cant,nFichero);
    printf("Vector rellenado\n" );
    float media=mediaAleatoriosPares(cant,vectorAleatoriosPares);
    printf("La media de los numeros pares es %2.f\n",media );
  }
  return 0;
}
